...
from .operator_utils import nchw_to, Format, l2norm
from einops_exts import rearrange_many
from .attention_utils import compute_freqs_cis, apply_rotary_emb
try:
...
# attention
class AttentionExperimental(nn.Module):
    def __init__(
        self,
        dim,
        heads = 8,
    ...
    def forward(self, x, 
        ...
        if self.is_decoder:
            assert x.shape[-2] % self.block_size == 0, 'sequence length must be divisible by block size'
            x = rearrange(x, 'b (l s) d -> b l s d', s=self.block_size)


        q = self.to_q(x)

        # add text conditioning, if present
        if self.use_self_text_cond:
        ...
        if self.is_decoder:
            sim = einsum('bsihd, bsjhd -> bshij', q, k) * self.scale
        else:
            sim = einsum('bihd, bjhd -> bhij', q, k) * self.scale
        # sim = einsum("... n d, ... m d -> ... n m", q, k) * self.scale

        if self.is_encoder:
            # TODO block causal exp
            # if mask is None: 
            #     mask = jnp.ones(x.shape[:2])
            # block_seq_inds = jnp.arange(mask.shape[-1]) // cfg.block_size
            # block_mask = nn.make_attention_mask(block_seq_inds, block_seq_inds, pairwise_fn=jnp.greater_equal)
            # mask = mask[:, None, None, :] * block_mask
            # TODO full causal exp
            if mask is None: 
                mask = jnp.ones(x.shape[:2])
            causal_mask = nn.make_causal_mask(mask)
            mask = mask[:, None, None, :] * causal_mask
        elif self.is_decoder:     
            if mask is None:
                mask = jnp.ones(x.shape[:3])
            else:
                mask = rearrange(mask, 'b (l s) -> b l s', s=self.block_size)
            mask = mask[..., None, None, :]
        else:
            if mask is None:
                mask = jnp.ones(x.shape[:2])
...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, 
        ...
        self.attn = AttentionExperimental(dim=hidden_size, 
                                          heads=num_heads,
...
class DiT(nn.Module):
    ...
    def forward(self, 
        ...
        # text condition
        context, text_mask = self.text_conditioner(text_embeds, text_mask, cond_drop_prob) if exists(text_embeds) else None
